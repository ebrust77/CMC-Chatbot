...
def format_for_display(text: str, style: str = "Bulleted", simplify: bool = True,
    ...
    # Normalize
    text = text.replace("\\n", "\n")
    text = re.sub(r"(?m)^\s*•\s+", "- ", text)  # normalize bullets to '-'

    # Simplify headings & spacing
    if simplify:
    ...
    if style == "Bulleted":
        ...
        for ln in text.splitlines():
            s = ln.strip()
            # Headers: ensure they are on their own line (not part of bullet list)
            if re.match(r"^\*\*.+\*\*$", s):
                if prev_was_bullet and out and out[-1] != "":
                    out.append("")  # <- blank line to break list
                out.append(s)
                prev_was_bullet = False
                section_count = 0
            # Bullets: cap words
                ...
                if section_count >= bullets_max:
                    out.append("")  # spacer after long list
                    section_count = 0
            # Paragraphs: keep but ensure they don't stick to bullets
            elif s:
                if prev_was_bullet and out and out[-1] != "":
                    out.append("")
                out.append(s)
        ...
        # Plain mode: remove markdown bullets/headings while keeping structure
...
with st.sidebar:
    ...
    if st.button("Insert example question"):
        ...
        examples = {
            ...
            "Report Results": "When can we use report-only results for Phase 1 cell therapy release tests?",
...
q = st.text_area("Ask a question", key="q", placeholder="Ask about Report Results, CRLs, APV, PPQ, comparability, specs, Module 3, stability, etc.", height=120)
...
def detect_intent(q: str) -> str:
    ...
    mapping = {
        ...
        "Report Results": ["report result","report-only","report only","rr","reporting results","without specs"],
...
def render_answer(intent, product, stage, region):
    ...
    for sec in ["Guidance Summary","What reviewers look for","Common pitfalls","Suggested next steps"]:
        if sec in blocks and (not included or sec in included):
            if lines:
                lines.append("")                 # ensure header is separated from prior bullets
            lines.append(f"**{sec}**")           # header on its own line
            for b in blocks[sec]:
                lines.append(f"- {b}")
            any_content = True
...
st.caption("Anchors: FDA CGT CMC (2020), Potency (2011), Draft Comparability (2023), Draft Potency Assurance (2023), ICH Q5E, Q2(R2)/Q14; ARM A‑Cell case study.")
